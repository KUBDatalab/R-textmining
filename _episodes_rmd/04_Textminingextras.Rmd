...
Stopword lists are avalable for a range of European languages via the tm package (see documentation on https://cran.r-project.org/web/packages/tm/tm.pdf p. 38)
...
```{r}
stemmed_text <- wordStem(column_in_tibble_with_text, language = "name of language")
```
...
Now you can quickly load it in next time by using `read_delim()`. Use `filter()` to filter your dataset however you like it. For more info on the `filter()` function, see https://r4ds.had.co.nz/transform.html chapter 5 section 2
